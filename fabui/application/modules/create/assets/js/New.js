...
var GCode_Analizer = function(url) {
	...
	this.init = function() {
		...
		self.get(function(response) {
			...
			self.g_code = response;
			self.normalize_gcode(); // call next step only when ajax is complete
	...
	this.normalize_gcode = function() {
		this.g_code = this.g_code.split(/\n/);
	}
	...
	this.wait = function( type ) {
		...
		if( type == 'gcode_ready')
			control = self.gcode_ready;
			
		if( type == 'parser_ready')
			control = self.parse_ready;
		...
		if (!control) {
			...
			if (!control) {
				...
				setTimeout(this.wait.bind(this), 10, type);
 ...
 * 
 */
GCode_Analizer.prototype.get_ready = function() {
 ...
 * 
 */
GCode_Analizer.prototype.do_parse = function() {

	// function do_parse(){
	console.log('do_parse');
	this.wait('gcode_ready');
...
GCode_Analizer.prototype.analyze_model = function() {

	// function analyze_model(){

	//wait
	console.log('analyze model');
	this.wait('parser_ready');
