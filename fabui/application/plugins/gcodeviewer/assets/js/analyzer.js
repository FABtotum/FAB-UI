/**
 * Created with JetBrains WebStorm. User: hudbrog Date: 2/14/13 Time: 1:15 PM
 */
...
GCODE.analyzer = (function() {
	...
	var multiplyPoint = function(point) {
		return {
			x : parseInt(point.x * transform.a + point.y * transform.c
					+ transform.e),
			y : parseInt(point.x * transform.b + point.y * transform.d
					+ transform.f)
		}
	}
	...
	var drawLineEnds = function(layerNum) {
		...
		if (layerNum < model.length) {
			...
			for ( var i = 0; i < model[layerNum].length; i++) {
				...
				if (typeof (cmds[i]) === 'undefined')
					continue;
				if (typeof (cmds[i].prevX) !== 'undefined'
				...
				if (cmds[i].extrude) {
					...
					// }
					var r2 = checkArea(imgData, {
						x : x,
						y : y
					});
					var r1 = checkArea(imgData, {
						x : prevX,
						y : prevY
					});
					var result = (r2 + r1) / 2;
					model[layerNum][i].errLevelE = r2 < usableLevel ? 0 : r2
							- stepLevel;
					model[layerNum][i].errLevelB = r1 < usableLevel ? 0 : r1
							- stepLevel;
					if (r1 < usableLevel && r2 < usableLevel)
						model[layerNum][i].errType = 3;
					else if (r1 < usableLevel)
						model[layerNum][i].errType = 1;
					else if (r2 < usableLevel)
						model[layerNum][i].errType = 2;
					else
						model[layerNum][i].errType = 0;
					// model[layerNum][i].errType =
					// r1<usableLevel&&r2<usableLevel?2:(r1<usableLevel||r2<usableLevel?1:0);
					if (model[layerNum][i].errType === 1) {
						model[layerNum][i].errDelimiter = findNearestPoint(
								imgData, x, y, prevX, prevY);
		...
		return {
			errors : totalErr,
			longestStreak : longestErrStreak
		};
	}

	var analyze = function(layerNum) {
		GCODE.renderer.render(layerNum, 0, GCODE.renderer
				.getLayerNumSegments(layerNum));
	...
	var analyzeStep = function() {
		...
		$('#analysisProgress').width(parseInt(progress) + '%').text(
				parseInt(progress) + '%');
	...
	var init = function() {
		...
		var p1 = multiplyPoint({
			x : 0,
			y : 0
		});
		var p2 = multiplyPoint({
			x : extrusionWidth,
			y : 0
		});
