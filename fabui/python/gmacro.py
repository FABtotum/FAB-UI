...
#pre_SCAN CHECK (SAFETY)
	...
	macro("G92 Z240","ok",3,"Setting Z position",0.1,verbose=False)
	...
	macro("M92 E"+str(units['e']),"ok",0.1,"Setting steps/units for 3D printing",0.5,verbose=False)
	macro("M18","ok",2,"Stopping motors",0.1,verbose=False)
...
#start print (valid for 3dprinting or milling)
	...
	macro("G0 X5 Y5 Z30 F1500","ok",3,"Move to oozing point",1)
	#pre heating M104 S0
	macro("M104 S180","ok",3,"Pre Heating - fast ",20)
	...
	macro("G27","ok",100,"Lowering the building platform",1) #normally Z=240mm
	...
	macro("M220 S100","ok",5,"reset speed override",0.1)
	macro("M106 S0","ok",5,"Turning Fan off",1) #should be moved to firmware
	macro("M18","ok",1,"Motor Off",1) #should be moved to firmware
elif preset=="end_print_additive":
	serial.flush()
	#note: movement here is done so it works with AUTO positioning (additive mode).
	trace("Terminating...",log_trace)
	macro("G0 X210 Y210 Z240 F10000","ok",10,"Moving to safe zone",0.1,verbose=False) #right top, normally Z=240mm
	#macro("M302","ok",10,"Aborting temp rise",1)
	macro("M104 S0","ok",5,"Shutting down Extruder",1)
	macro("M140 S0","ok",5,"Shutting down Heated Bed",1)
	macro("M220 S100","ok",2,"reset speed override",0.1)
	macro("M106 S0","ok",5,"Turning Fan off",1) #should be moved to firmware
	macro("M18","ok",1,"Motor Off",1) #should be moved to firmware
	...
	macro("M140 S75", "ok",90,"Heating Bed - fast ",1)
	...
	#macro("G0 Z40","TRIGGERED",2,"Front panel door control",5)
#Auto bed leveling
elif preset=="auto_bed_leveling":
	trace("Auto Bed leveling Initialized",log_trace)
	macro("G91","ok",2,"setting relative position",1)
	#macro("G0 X17 Y61.5","ok",1,"Offset",1)
	macro("G0 Z25","ok",2,"moving away from the plane",1)
...
#p_scan preset
	...
	trace("checking panel door status and bed inserted",log_trace)
	macro("M741","TRIGGERED",2,"Front panel door control",0.1)
	macro("M744","open",2,"Building plane has been tilted or removed!",0.1, warning=True)
...
#unload spool
	...
	macro("G27 Z240","ok",100,"zeroing Z axis",1,verbose=False)
	macro("G0 X210 Y210 Z150 F10000","ok",10,"Moving to safe zone",0.1,verbose=False) #right top corner Z=240mm
	...
	#heating nozzle
	macro("M109 S220","ok",120,"Heating Nozzle... Get ready to pull gently...",0.1)
	macro("M728","ok",1,"Start pulling gently!",3)
...
#load spool from reel
	...
	macro("G0 E110 F200","ok",1,"Entering heating chamber (slow)",5)
...
#probe calibration
	...
	####
	macro("G90","ok",2,"Abs_mode",1)
	macro("G0 Z50 F1000","ok",3,"Moving plane away",1)
	macro("G28","ok",90,"homing all axis",1)
