    ...
    def _listen(self):
        ...
        while self._listen_can_continue():
			...
			########################		
				...
				#try:
                #self.temperatures['extruder']=line.split( )[1]
                #self.temperatures['extruder']=self.temperatures['extruder'].split(":")[1]
                #self.temperatures['bed']=line.split( )[3] 
                #self.temperatures['bed']=self.temperatures['bed'].split(":")[1]


                matches = re.findall('ok T:(\d+\.?\d*)', line)
                if matches:
                    self.temperatures['extruder'] = matches[0]
                matches = re.findall('ok T:(\d+\.?\d*) /(\d+\.?\d*)', line)
                if matches:
                ...
                #print "TEMPS: " + str(self.temperatures)
            ...
            if line.startswith('ok') and "T:" in line and self.tempcb:
                # callback for temp, status, whateve
