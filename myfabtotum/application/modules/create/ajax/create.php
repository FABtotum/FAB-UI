...
$_macro_response             = '/var/www/temp/'.$_macro_function.'_'.$_time.'.log';
$_start_print_macro_response = '/var/www/temp/star_print_'.$_time.'.trace';
...
write_file($_start_print_macro_response, '', 'w');
chmod($_start_print_macro_response, 0777);

/** PRESET AUTO-BED-LEVELING MACRO */

if(!$_skip_abl && $_print_type == 'additive'){
    
    //$_macro_function = !$_skip_abl ? 'auto_bed_leveling' : 'zero_all';
    $_macro_function = 'auto_bed_leveling';
   
    //$_macro_trace    = '/var/www/temp/'.$_macro_function.'_'.$_time.'.trace';
    
	$_command_macro  = 'sudo python /var/www/myfabtotum/python/gmacro.py '.$_macro_function.' '.$_macro_trace.' '.$_macro_response;
...
if($_print_type == 'additive'){
    ...
    /** DEFAULT START PRINT MACRO */
    $_command_start_print_macro  = 'sudo python /var/www/myfabtotum/python/gmacro.py start_print '.$_macro_trace.' '.$_start_print_macro_response;
    ...
    /** WAIT MACRO TO FINISH */
    while(str_replace('<br>', '', file_get_contents($_start_print_macro_response)) == ''){   
        sleep(0.5);
    }
    
    /** CHECK MACRO RESPONSE */
    if(str_replace('<br>', '', file_get_contents($_start_print_macro_response)) != 'true'){
    
    	//unlink($_start_print_macro_response);
        //unlink($_macro_trace);
        header('Content-Type: application/json');
        echo json_encode(array('response' => false, 'message' => str_replace(PHP_EOL, '<br>', file_get_contents($_macro_trace)), 'response_text' => file_get_contents($_start_print_macro_response)));
        exit();
    }
...
/** START PROCESS */
$_command        = 'sudo python /var/www/myfabtotum/python/gpusher.py '.$_file['full_path'] .' '.$_monitor_file .' '.$_data_file.' '.$_time_monitor.' '.$_trace_file.' '.$id_task.' 2>'.$_debug_file.' > /dev/null & echo $!';
