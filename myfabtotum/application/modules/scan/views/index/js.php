...
<?php endforeach; ?>
/* SCAN QUALITY */ 
var scan_quality = <?php echo  $_task  ? $_task_attributes->quality : 0 ?>;
/* SCAN QUALITY INFO */
...
var scan_uri                = '<?php echo  $_task  ? $_task_attributes->scan_uri : '' ?>';
var pprocess_pid            =  <?php echo  $_task  ? $_task_attributes->pprocess_pid : 0 ?>;
var pprocess_monitor_file   = '<?php echo  $_task  ? $_task_attributes->folder.$_task_attributes->pprocess_monitor : '' ?>';
var pprocess_monitor_folder = '<?php echo  $_task  ? $_task_attributes->folder : '' ?>';
/* MONITORING SCAN INFO */
var scan_finished            = <?php echo  $_task  ? $_scan_monitor->scan->completed == 1 ? 'true' : 'false' : 'false' ?>;
...
var scan_stats_file = '<?php echo $_task ? $_task_attributes->scan_stats_file : '' ?>';
<?php 
if($_task){
...
var pprocess_finished = <?php echo  $_task  ? $_pprocess_monitor->post_processing->completed == 1 ? 'true' : 'false' : 'false' ?>;
...
var pprocess_stats_file = '<?php echo $_task ? $_task_attributes->pprocess_stats_file : '' ?>';
<?php 
if($_task){
...
var interval_timer; 
var precision        = 3;
var current_step     = <?php  echo $_task ? $_task_attributes->step : 0?>;
var monitor_count    = 0;
...
function start(){
	...
	$.ajax({
		  ...
		  data: { mode: scan_mode, quality: scan_quality, x1: x1, x2: x2, y1:y1, y2:y2, axis_increment: axis_increment, start_degree: start_degree, end_degree: end_degree, density:density  },
		  dataType: 'json',
		  asynch: true,
		  beforeSend: function() {
			    openWait('Initializiang scan..');
	      }
	...
	if(!scan_finished || (!ISPROBING && !pprocess_finished)){	
		...
		$.ajax({
           ...
           /* try{ */ 
    			...
    			/*se il numero dell'immagine è minore al numero degli slice totali mostro l'immagine, evito che di caricare un immagine non esistente*/
    			if(parseInt(scan_monitor_response.scan.stats.img_number) <= parseInt(scan_quality_info[scan_quality].values.slices)){
    				...
    				/*array_scan_image.push(scan_monitor_response.scan.stats.img_number);*/
