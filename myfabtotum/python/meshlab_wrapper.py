...
#sudo pyhton meshlab_wrapper.py -t/var/www/myfabtotum/slic3r/meshlab.trace -l/var/www/temp/meshlab.log -i/var/www/myfabtotum/slic3r/file.asc -o/var/www/myfabtotum/slic3r/output.stl -s/root/meshlab_script.mlx
...
def printlog(percent,status):		
	str_log='{"Meshing":{"id": "'+str(task_id)+'","pid": "'+str(myPID)+'","started": "'+str(basetime)+'","completed": "'+str(completed)+'","completed_time": "'+str(endtime)+'","stats":{"percent":"'+str(percent)+'","time_left":"'+str(eta)+'","time_elapsed":"'+str(elapsed)+'","time_total":"'+str(estimate)+'"}}}'
...
call (['sudo php /var/www/myfabtotum/script/finalize.php '+str(task_id)+ " mesh"], shell=True)
