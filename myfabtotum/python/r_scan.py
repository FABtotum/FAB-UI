...
post=1       #post mode 1=default:laser switch and greyscale 2=greyscale only 3:laser only.
...
#serial.write('G92 E0\r\n')  #set extruder to zero
serial.write('M92 E1914.5295\r\n')     #set extruder as A axis.
...
time.sleep(begin*0.03+z_offset*0.01)  #take its time to move
...
#shell call raspistill
	...
	call (["raspistill -hf -vf -rot 90 -cfx 128:128 --exposure off -awb sun -ISO " + str(iso) + " -w "+ str(height) +" -h "+ str(width) +" -o " + scanfile + " -t 0"], shell=True)
...
while (i <= slices) :
	...
	time.sleep(deg*0.03)  #take its time to rotate
	serial.write('M700 S50\r\n') #turn laser ON
