...
trace("HEATING TEST (temperature in 60 seconds): " ,log_trace)
...
if len(serial_reply)>5:
	...
	ext_temp=-1
	bed_temp=-1
	trace("FAILURE : Extruder Sensor not responding after 60 seconds )",log_trace)
	s_error+=1
	summary() #self test has detected a big problem! exit!

if ext_temp>=190:
	trace("OK : Extruder heating(" + ext_temp +" in 60 seconds )",log_trace)
else:
	trace("FAILED : Extruder heating (" + serial_reply + ") took more than 60 seconds",log_trace)
	summary()
	
# 2 - Bed turns On, reach temp, turns off

if float(bed_temp)>=50: #reach this at least in 60 seconds
	trace("OK : Bed heating("+ str(bed_temp) +") in 60 seconds" + serial_reply,log_trace)
else:
	trace("FAILED : Bed heating took more than 60 seconds ( currently : " + str(bed_temp)+")",log_trace)
	s_error+=1
	summary()
