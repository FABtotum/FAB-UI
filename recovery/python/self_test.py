...
start_ext = float(ext_temp)
start_bed = float(bed_temp)

if float(ext_temp)>0:
    trace("OK : Extruder sensor ( " + str(start_ext) +"&deg;C ) responding correctly",log_trace)
else:
    trace("FAILED : Extruder sensor disconnected (" + str(start_ext) + "&deg;C ) ",log_trace)
    summary()
...
trace("Please wait that test temperatures are reached", log_trace)
...
while ((ext_temp <= 190) and (time_elapsed < temperature_timeout)):
...
if ext_temp>=190:
    trace("OK : Extruder heating ( started from " + str(int(start_ext)) + "&deg;C reached temperature " + str(int(ext_temp)) + "&deg;C in "+str(int(time_elapsed))+" seconds )",log_trace)
else:
    trace("FAILED : Extruder heating ( reached " + str(int(ext_temp)) + "&deg;C ) took more than "+str(temperature_timeout)+" seconds, please check head connections",log_trace)
    summary()
